...
class down_email():
    ...
    def run_ing(self):
        ...
        try:
            #telnetlib.Telnet(self.pop3_server, 110)
            #self.server = poplib.POP3_SSL(self.pop3_server, 110, timeout=10)
            self.server = poplib.POP3(email_server)
        ...
        # for i in range(1, index + 1):# 顺序遍历邮件
            ...
            if From.__contains__('tsinghua.edu.cn') or \
                From.__contains__('no-reply') or \
                From.__contains__('helpdesk'):
                continue
            print(From)
            #To = parseaddr(msg.get('To'))[1]
            body_pre = msg.get_payload()
            if body_pre:
                if isinstance(body_pre, str):
                    wf.write(From + ':' + body_pre + '\n')
                else:
                    while isinstance(body_pre, list):
                        body_struct = body_pre[0]
                        body_pre = body_struct._payload
                    body = body_pre.replace('\n', '\r')
                    wf.write('<MyEmailDelimiter>' + From + '<MyEmailDelimiterSub>' + body)
...
if __name__ == '__main__':
    ...
    try:
        # 输入邮件地址, 口令和POP3服务器地址:
        user = 'slt20@mails.tsinghua.edu.cn'
        # 此处密码是授权码,用于登录第三方邮件客户端
        password = 'sltthu123456'
        #email_server = 'pop.mails.tsinghua.edu.cn'
        email_server = 'mails.tsinghua.edu.cn'
        # user='xinfei@.com'
        # password = 'f67h2'
        # eamil_server = 'pop.exmail.qq.com'
        email_class=down_email(user=user,password=password,email_server=email_server)
        email_class.run_ing()
    except Exception as e:
        import traceback
