...
from utils_v2 import GetPfx2ASByRv, GetLongestMatchPrefixByRv, GetAsListOfIpByRv, GetAsStrOfIpByRv, \
    ...
    GetSibRelByMultiDataFiles, ClearSibRelByMultiDataFiles, IsSibByIRR_2, SelAsnInBgpPathList, CompressAsPath, \
    CompressAsPathToMin
...
from constr_irr_db import ConnectToIrrDb, ConstrIrrCache, GetIrrOrgFromDb, CloseIrrDb, IniIrrCache, PreGetNonOverlayIpRanges
...
def ChgTrace2ASPath(trace_file_name, record_file_name, open_mode):
    global cur_map_method
    rf = open(trace_file_name, 'r', encoding='utf-8')
    #w_path_set_filename = str('res_as_path_has_set_of_prefix_') + trace_file_name
    w_path_set_filename = '1_has_set'
    wf_set = open(w_path_set_filename, 'w', encoding='utf-8')
    count_set = 0
    #w_path_loop_filename = str('res_as_path_has_loop_of_prefix_') + trace_file_name
    w_path_loop_filename = '2_has_loop'
    wf_loop = open(w_path_loop_filename, 'w', encoding='utf-8')
    count_loop = 0
    #w_single_path_filename = str('res_single_as_path_of_prefix_') + trace_file_name
    w_single_path_filename = '3_single_path'
    wf_single = open(w_single_path_filename, 'w', encoding='utf-8')
    count_single = 0
    #w_multi_path_filename = str('res_multi_as_path_of_prefix_') + trace_file_name
    w_multi_path_filename = '4_multi_path'
    wf_multi = open(w_multi_path_filename, 'w', encoding='utf-8')
    count_multi = 0
    w_path_ixp_ip_filename = '5_has_ixp_ip'
    wf_ixp = open(w_path_ixp_ip_filename, 'w', encoding='utf-8')
    count_ixp = 0
    w_nobgp_filename = '0_no_bgp'
    wf_nobgp = open(w_nobgp_filename, 'w', encoding='utf-8')
    count_nobgp = 0
    count_total = 0

    trace_dict = dict()
    trace_dict_ixp = dict() #分开统计
    curline = rf.readline()
    while curline:
        if not curline.startswith('T'):
            curline = rf.readline()
        ...
        if as_path.__contains__('{'):
            ...
            wf_set.write("[%s]%s\n" %(dst_key, ip_path))
            count_set += 1
        elif HasLoop(as_path):
            wf_loop.write("[%s]%s\n" %(dst_key, ori_as_path))
            wf_loop.write("[%s]%s\n" %(dst_key, ip_path))
            count_loop += 1
        else:      
            if as_path.__contains__('<'):
                if dst_key not in trace_dict_ixp.keys():
                    trace_dict_ixp[dst_key] = []
                trace_dict_ixp[dst_key].append([ip_path, ori_as_path])
            else:  
                if dst_key not in trace_dict.keys():
                    trace_dict[dst_key] = []
                trace_dict[dst_key].append([ip_path, ori_as_path])
            ...
            for cur_min_path in trace_dict[dst_key].keys():
                if as_path == cur_min_path or FstTraceContainsSndTrace(cur_min_path, as_path):
                    find = True
                    break
                elif FstTraceContainsSndTrace(as_path, cur_min_path):
                    trace_dict[dst_key][as_path] = [ip_path, ori_as_path]
    ...
    for (dst_key, ip_as_path_list) in trace_dict.items():    
        #multi_num += FilterSimilarPathAndRecord(dst_key, ip_as_path_list, wf_single, wf_multi, wf_ixp_index, False)
        res = FilterSimilarPathAndRecord(dst_key, ip_as_path_list, wf_single, wf_multi, False)
        if res == 1:
            count_single += res
        else:
            count_multi += res
    ...
    for (dst_key, ip_as_path_list) in trace_dict_ixp.items():    
        #multi_num += FilterSimilarPathAndRecord(dst_key, ip_as_path_list, wf_single, wf_multi, wf_ixp_index, False)
        res = FilterSimilarPathAndRecord(dst_key, ip_as_path_list, wf_ixp, wf_ixp, False)
        count_ixp += res
    wf_ixp.close()

    wf = open(record_file_name, open_mode)
    wf.write("Total valid trace num: %d\n" %count_total)
    wf.write("no_bgp num: %d\n" %count_nobgp)
    wf.write("set num: %d\n" %count_set)
    wf.write("loop num: %d\n" %count_loop)
    wf.write("single num: %d\n" %count_single)
    wf.write("multi num: %d\n" %count_multi)
    wf.write("ixp num: %d\n" %count_ixp)
    wf.close()
        ...
        if not find:
            if not update_flag or not update_flag[i]:
                ...
                if cur_map_method.__contains__('midar'):
                    ...
                    if mi_as and mi_as not in trace_list[i].split('_'):
                        if trace_list[i] != '?':
                ...
                if cur_map_method.__contains__('bdrmapit'):
                    ...
                    if bdr_as and bdr_as not in trace_list[i].split('_'):
                        if trace_list[i] != '?':
...
#原来的版本可能有bug，重新写
#2021.7.6将和bgp path匹配上的trace hop修正为对应的bgp hop，减少map到AS的数量
def TracePathIsNormal(ori_trace_path, ori_bgp_path, ip_path, use_geo_data):
    ...
    for i in range(0, len(ori_trace_path_list)):
        ...
        if cur_hop.__contains__('*') or cur_hop.__contains__('?') or cur_hop.__contains__('<'):
            match_trace_list.append(cur_hop)
            continue
        match_hop = SelAsnInBgpPathList(cur_hop, bgp_path_list)
        if match_hop != '':
        #if AsnInBgpPathList(cur_hop, bgp_path_list):
            bgp_prev_index = FindTraceAsInBgpPath(match_hop, bgp_path_list)
            ip_prev_index = i
            ...
            while bgp_next_index < len(bgp_path_list):
                if AsnInTracePathList(bgp_path_list[bgp_next_index], ori_trace_path_list):
                    break
                bgp_next_index += 1
            match_trace_list.append(match_hop)
            continue  #正常，看下一跳
        ip_next_index = i + 1
        ...
        if IsSib_2(bgp_prev_hop, cur_hop):  #和左边正常的hop是sibling关系
            match_trace_list.append(bgp_prev_hop)
            continue    #正常，看下一跳
        ...
        if bgp_next_index < len(bgp_path_list):
            ...
            if IsSib_2(bgp_next_hop, cur_hop):   #和右边的hop是sibling关系或者离得特别近，可能是IXP引起的不一致
                match_trace_list.append(bgp_next_hop)
                continue    #正常，看下一跳
        #if False:   #2021.4.29 先不考虑地理位置
        if ip_next_index < len(ip_path_list) and use_geo_data:
            ...
            if res and dist < min_dist_limit:   #和右边的hop离得特别近，可能是IXP引起的不一致
                ...
                match_trace_list.append(ori_trace_path_list[ip_next_index])
                continue    #正常，看下一跳
        normal = False
        if bgp_prev_index + 1 < bgp_next_index:
            for j in range(bgp_prev_index + 1, bgp_next_index):
                ...
                if IsSib_2(bgp_hop, cur_hop):   #和对应的bgp hop是sibling关系
                    match_trace_list.append(bgp_hop)
                    normal = True    #正常，看下一跳
                    break
            ...
            if not normal and use_geo_data:
                if False:
                    ...
                    normal = True
        if not normal:
            for cur_hop in ori_trace_path_list[i:]:
                match_hop = SelAsnInBgpPathList(cur_hop, bgp_path_list)
                if match_hop != '':
                    match_trace_list.append(match_hop)
                else:
                    ...
                    for elem in cur_hop.split('_'):
                        tmp_set.add(elem)
                    match_trace_list.append('_'.join(list(tmp_set)))
            dif_size = DifOfTraceAndBgp(CompressAsPathToMin(ori_bgp_path), CompressAsPathToMin(CompressAsPath(ori_trace_path)))
            #return (False, len(bgp_path_list) - bgp_prev_index, ' '.join(match_trace_list))    #有一跳不正常，即为不正常，把有几处不一致的计算方法进行修改
            return (False, dif_size, ' '.join(match_trace_list))    #有一跳不正常，即为不正常，把有几处不一致的计算方法进行修改
    return (True, 0, ' '.join(match_trace_list))
    
def TracePathIsNormalByIRRData(ori_trace_path, ori_bgp_path, ip_path, check_irr, check_newer_org, wf_join_org = None):
    global cur_map_method
    match_trace_list = []
    bgp_path_list = CompressAsPathToMin(ori_bgp_path).split(' ')
    ori_trace_path_list = ori_trace_path.split(' ')
    ...
    for i in range(0, len(ori_trace_path_list)):
        ...
        if match_hop != '':
            ...
            match_trace_list.append(match_hop)
            continue  #正常，看下一跳
        #print(ip_path_list[i])
        bgp_prev_hop = bgp_path_list[bgp_prev_index]
        #print(bgp_prev_hop)
        #print(check_irr)
        if check_irr and IsSibByIRR(bgp_prev_hop, ip_path_list[i], wf_join_org):  #和左边正常的hop是sibling关系
            match_trace_list.append(bgp_prev_hop)
            continue    #正常，看下一跳
        if check_newer_org and IsSibByMultiDataFiles_2(bgp_prev_hop, cur_hop, wf_join_org):
            match_trace_list.append(bgp_prev_hop)
            continue
        if bgp_next_index == 0: #abnormal
            print(ori_trace_path)
        ...
        if bgp_next_index < len(bgp_path_list):
            bgp_next_hop = bgp_path_list[bgp_next_index]
            if check_irr and IsSibByIRR(bgp_next_hop, ip_path_list[i], wf_join_org):  #和右边正常的hop是sibling关系
                match_trace_list.append(bgp_next_hop)
                continue    #正常，看下一跳
            if check_newer_org and IsSibByMultiDataFiles_2(bgp_next_hop, cur_hop, wf_join_org):
                match_trace_list.append(bgp_next_hop)
                continue
        normal = False
        if bgp_prev_index + 1 < bgp_next_index:
            for j in range(bgp_prev_index + 1, bgp_next_index):
                bgp_hop = bgp_path_list[j]
                if check_irr and IsSibByIRR(bgp_hop, ip_path_list[i], wf_join_org):   #和对应的bgp hop是sibling关系
                    match_trace_list.append(bgp_hop)
                    normal = True    #正常，看下一跳
                    break
                if check_newer_org and IsSibByMultiDataFiles_2(bgp_hop, cur_hop, wf_join_org):   #和对应的bgp hop是sibling关系
                    match_trace_list.append(bgp_hop)
                    normal = True    #正常，看下一跳
                    break
        if not normal:
...
# def ModiTraceBasedOnBgp(ori_trace_path, bgp_path):
#     modi_trace_path = ''
#     for cur_hop in ori_trace_path.split(' '):
#         if cur_hop.__contains__('*') or cur_hop.__contains__('?') or cur_hop.__contains__('<'):
#             modi_trace_path += ' ' + cur_hop
#         else:
#             match_hop = SelAsnInBgpPathList(cur_hop, bgp_path.split(' '))
#             if match_hop != '':        
#                 modi_trace_path += ' ' + match_hop
#             else: #未匹配，按原来的记录
#                 modi_trace_path += ' ' + cur_hop
#     return modi_trace_path

def CheckTraces(filename, use_mi_data, use_geo_data, save_normal_trace = False):
    rf = open(filename, 'r')
    wf_ab = open(filename + '_ab', 'w')
    wf_unmap = open(filename + '_unmap', 'w')
    wf_normal = None
    if save_normal_trace:
        wf_normal = open(filename + '_normal', 'w')
    count_ab = 0
    count_unmap = 0
    count_total = 0
    ...
    while curline:
        ...
        for ori_bgp_path in bgp_path_list:
            (normal, dif_size, match_trace_path) = TracePathIsNormal(ori_trace_path_2, ori_bgp_path, ip_path, use_geo_data)
            if normal:
        ...
        #modi_trace_path = ModiTraceBasedOnBgp(ori_trace_path, ori_bgp_path)
        if not normal:
            #wf_ab.write("[%s] %s\n" %(dst_key, ori_trace_path_2))
            wf_ab.write("[%s] %s\n" %(dst_key, match_trace_path))
            wf_ab.write("\t\t%s\n" %similar_bgp_path)
...
def CheckTracesByIRRData(filename, check_irr, check_newer_org, wf_join_org = None):
    rf = open(filename, 'r')
    wf_ab = open(filename + '_ab', 'w')
    wf_unmap = open(filename + '_unmap', 'w')
    wf_normal = open(filename + '_normal', 'w')
    ...
    while curline:
        ...
        normal = False
        #print("%s%s%s" %(curline, curline_bgp, curline_ip))
        (normal, dif_size, match_trace_path) = TracePathIsNormalByIRRData(ori_trace_path, ori_bgp_path, ip_path, check_irr, check_newer_org, wf_join_org)
        if not normal:
            wf_ab.write("[%s] %s\n" %(dst_key, match_trace_path))
            wf_ab.write("%s%s" %(curline_bgp, curline_ip))
            count_ab += 1
        elif match_trace_path.__contains__('?'):
            wf_unmap.write("[%s] %s\n" %(dst_key, match_trace_path))
            wf_unmap.write("%s%s" %(curline_bgp, curline_ip))
        else:
            wf_normal.write("[%s] %s\n" %(dst_key, match_trace_path))
            wf_normal.write("%s%s" %(curline_bgp, curline_ip))
        curline = rf.readline()
...
#                 update_flag[i] = True
#             (normal, dif_size, match_trace_path) = TracePathIsNormal(' '.join(ori_trace_list), ori_bgp_path, ip_path, True)
#             if normal:
#                 break
#             if min_dif_size > dif_size:
#                 similar_bgp_path = ori_bgp_path
...
#                             (match, dif_size, match_trace_path) = TracePathIsNormal(as_trace, bgp_path, curline_ip.split(']')[1].strip('\n').strip(' '), False)
#                             if match:
#                                 break
#                             if min_dif_size > dif_size:
...
def CheckAbPathByDstAs(filename, w_filename):
    ...
    while curline_trace:
        ...
        for dst_as in dst_as_str.split('_'):
            ...
            for ori_bgp_path in bgp_path_list:
                (normal, dif_size, match_trace_path) = TracePathIsNormal(ori_trace_path, ori_bgp_path, '', False)
                if normal:
                    break
            if normal:
                break
...
def FilterOneAbBetweenSameNormal(filename):
    ...
    while curline_trace:
        ...
        for i in range(0, len(ori_trace_path_list)):
            ...
            if match_hop != '':
                ...
                if bgp_prev and bgp_next and (bgp_prev & bgp_next):
                    match_trace_list.append('_'.join(list(bgp_prev & bgp_next))) #左右都map到了bgp_path中的同一跳，该异常跳可能是map出错
                else: #不正常，记录
                    normal = False
                    for cur_hop in ori_trace_path_list[i:]:
                        match_hop = SelAsnInBgpPathList(cur_hop, bgp_path_list)
        ...
        if not normal:  
            ...
            wf_normal.write("[%s] %s\n" %(dst_key, ' '.join(match_trace_list)))
            wf_normal.write(curline_bgp)
            wf_normal.write(curline_ip)
        curline_trace = rf.readline()
...
#2021.7.20 bgp path和trace path相似程度的判定，单领出来一个函数来定义相似程度
#以trace path中ab hop的数目判断不同的程度
def DifOfTraceAndBgp(bgp_path, trace_path):
    ab_num = 0
    bgp_path_list = bgp_path.split(' ')
    index = 0
    for cur_hop in trace_path.split(' '):
        if cur_hop.__contains__('*') or cur_hop.__contains__('?') or cur_hop.__contains__('<'):
            continue
        index += 1
        if not AsnInBgpPathList(cur_hop, bgp_path_list):
            ab_num += 1
    return ((ab_num << 8) + 255 - index) #这里使用两个评判标准，首先选取ab trace hop最少的，如果ab trace hop相同，选前面正常的长度最长的

def FindSimilarestBgpPath(trace_path, bgp_path_list):
    min_ab_num = 0xFFFF
    similarest_path = ''
    for ori_bgp_path in bgp_path_list:
        bgp_path = CompressAsPath(ori_bgp_path)
        ab_num = DifOfTraceAndBgp(bgp_path, trace_path)
        if ab_num < min_ab_num:
            min_ab_num = ab_num
            similarest_path = bgp_path
    return similarest_path

def FindSimilarestBgpPathForFile(filename, w_filename):
    rf = open(filename, 'r')
    wf = open(w_filename, 'w')
    curline_trace= rf.readline()
    ...
    while curline_trace:
        curline_bgp = rf.readline()
        curline_ip = rf.readline()
        ori_trace_path = curline_trace.split(']')[1].strip(' ').strip('\n')
        trace_path = CompressAsPathToMin(CompressAsPath(ori_trace_path))
        dst_prefix = curline_trace[1:curline_trace.index(' ')]
        bgp_path_list = GetBgpPathFromBgpPrefixDict_2(dst_prefix)
        similarest_path = FindSimilarestBgpPath(trace_path, bgp_path_list)
        wf.write(curline_trace)
        wf.write("\t%s\n" %similarest_path)
        wf.write(curline_ip)
        curline_trace = rf.readline()
...
def LooseCheck(vps, year, month, wf_join_org = None):
    #steps = ['norm_trace_asn', 'find_similarest_bgp', 'check_use_newer_org_data', 'check_use_irr', 'posb_ixp', 'oneab_betw_samenorm'] #2021.7.11，'same_dst_as'几乎没有过滤掉trace，而且即使匹配也不能说明trace正常，这里不再检查
    steps = ['posb_ixp', 'oneab_betw_samenorm']
    year_str = str(year)
    month_str = str(month).zfill(2)
    date = year_str + month_str + '15'
    ...
    ConnectToDb()
    SetCurMidarTableDate(year, month)
    if steps.__contains__('check_use_irr') or steps.__contains__('check_use_newer_org_data'):
        #print('here')
    ...
    for vp in vps:
        ...
        if steps.__contains__('norm_trace_asn'): #step 0
            ...
            NormTraceAS('final_normal', 'final_normal0')
            NormTraceAS('final_unmap', 'final_unmap0')
            
        if steps.__contains__('find_similarest_bgp'): #step 1
            print('find_similarest_bgp')
            TagStepInRecordFile(record_file_name3, 'find_similarest_bgp')  
            FindSimilarestBgpPathForFile('final_ab0', 'final_ab1')
            
        if steps.__contains__('check_use_newer_org_data'): #step 2
            ...
            (count_ab, count_total) = CheckTracesByIRRData('final_ab1', False, True, wf_join_org)
            w_record.write("In check_use_newer_org_data, ab num: %d, ab precent: %.2f\n" %(count_ab, count_ab / count_total))
            w_record.close()
            shutil.copyfile('final_ab1_ab', 'final_ab2')

        if steps.__contains__('check_use_irr'): #step 3 #严格来讲不算loose_check
            print('check_use_irr')
            TagStepInRecordFile(record_file_name3, 'check_use_irr') 
            w_record = open(record_file_name3, 'a')
            (count_ab, count_total) = CheckTracesByIRRData('final_ab2', True, False, wf_join_org)
            w_record.write("In check_use_irr, ab num: %d, ab precent: %.2f\n" %(count_ab, count_ab / count_total))
            w_record.close()
            shutil.copyfile('final_ab2_ab', 'final_ab3')

        if steps.__contains__('posb_ixp'): #step 4
            print('posb_ixp')
            TagStepInRecordFile(record_file_name3, 'posb_ixp')  
            # w_record = open(record_file_name3, 'a')
            # (count_ab, count_unmap, count_total) = CheckTraces('final_ab3', False, True, True) #检查可能的IXP
            # w_record.write("In posb_ixp, ab num: %d, ab precent: %.2f\n" %(count_ab, count_ab / count_total))
            # print("In posb_ixp, ab num: %d, ab precent: %.2f" %(count_ab, count_ab / count_total))
            # w_record.close()
            # shutil.copyfile('final_ab3_ab', 'final_ab4')
            shutil.copyfile('final_ab3', 'final_ab4') #2021.9.27 不要posb_ixp这一步，因为midar的很多数据都是错的

        if steps.__contains__('oneab_betw_samenorm'): #step 5
            ...
            w_record = open(record_file_name3, 'a')
            (count_ab, count_unmap, count_total) = FilterOneAbBetweenSameNormal('final_ab4')
            ...
            w_record.close()
            shutil.copyfile('final_ab4_ab', 'final_ab5')

        #2021.7.11，'same_dst_as'几乎没有过滤掉trace，而且即使匹配也不能说明trace正常，这里不再检查
        ...
        #     w_record.close()
        os.system('cat final_normal0 final_*_normal > final_normal_fin')
        os.system('cat final_unmap0 final_*_unmap > final_unmap_fin')
        os.system('cp final_ab5 final_ab_fin')
...
def RunOneDate(vps, year, month, map_methods):
    #steps = ['step 1', 'step 2', 'step 3', 'step 4', 'close_step']
    steps = ['step 3', 'step 4', 'close_step']
    year_str = str(year)
    month_str = str(month).zfill(2)
    date = year_str + month_str + '15'
    ...
    for vp in vps:
        #os.system("copy ..\\%s %s" %(trace_file_name, trace_file_name))
        #os.system("copy ..\\bgp_%s bgp_%s" %(g_asn, g_asn))
        trace_file_name = global_var.par_path + global_var.traceroute_dir + vp + '.' + date
        ...
        for tmp_map_method in map_methods:
            ...
            InitMidarCache()
            InitBdrCache()
            if cur_map_method.__contains__('bdrmapit2'):  #bdrmapit2将trace group到一年，按vp分
                if year < 2018: #只跑2018年以后的数据
                    os.chdir('..')
                    continue
                ConnectToBdrMapItDb(global_var.par_path + global_var.out_bdrmapit_dir + 'bdrmapit_' + vp + '_' + year_str + '.db')
                #print('here')
                #print(global_var.par_path + global_var.out_bdrmapit_dir + 'bdrmapit_' + vp + '_' + year_str + '.db')
                ConstrBdrCache()
            elif cur_map_method.__contains__('bdrmapit3'):  #bdrmapit3将所有vp的trace group到一年
                if year < 2018: #只跑2018年以后的数据
                    os.chdir('..')
                    continue
                ...
                if year == 2016 and month < 11: #2016年7-10月的bdrmapit没有跑出数据
                    os.chdir('..')
                    continue
                bdrmapit_filename = global_var.par_path + global_var.out_bdrmapit_dir + 'bdrmapit_' + vp + '_' + date + '.db'
                ConnectToBdrMapItDb(bdrmapit_filename)
                ConstrBdrCache()
            
            #map ip2AS
            if steps.__contains__('step 1'):
                print('step 1')
                if cur_map_method.__contains__('_') and os.path.isfile('../ribs/0_no_bgp'): #联合方法，且前面已经有ribs记录，这里直接copy，不用再计算
                    os.system("cp ../ribs/0_no_bgp .")
                    os.system("cp ../ribs/1_has_set .")
                    os.system("cp ../ribs/2_has_loop .")
                    os.system("cp ../ribs/3_single_path .")
                    os.system("cp ../ribs/4_multi_path .")
                    os.system("cp ../ribs/5_has_ixp_ip .")
                    wf = open(record_file_name, 'w')
                    rf = open('../ribs/record_' + vp + '.' + date + '_ribs', 'r')
                    curline = rf.readline()
                    while not curline.__contains__('step 2'):
                        wf.write(curline)
                        curline = rf.readline()
                    rf.close()
                    wf.close()
            ...
            if steps.__contains__('step 2'):
                print('step 2')
                TagStepInRecordFile(record_file_name, 'step 2')
                if cur_map_method.__contains__('_'):
                    #在ribs外用midar或bdrmapit map，过滤multi_path
                    filename = '4_multi_path'
                    CheckMultiPathByMi(filename, record_file_name, 'a')
                    files = ['3_single_path', '4_multi_path_single']
                    #files = ['4_multi_path_single_ab', '4_multi_path_multi_ab', '5_has_ixp_ip_ab_ab']
                    CombineFiles(files, '3_single_path_fin')
                    shutil.copyfile('4_multi_path_multi','4_multi_path_fin')                
                else:
                    shutil.copyfile('3_single_path','3_single_path_fin')
                    shutil.copyfile('4_multi_path','4_multi_path_fin')
            ...
            if steps.__contains__('step 3'):
                ...
                files = ['3_single_path_fin', '4_multi_path_fin', '5_has_ixp_ip']
                for filename in files:
                    (count_ab, count_unmap, count_total) = CheckTraces(filename, False, False, True) #step 2
                    if count_total > 0:
                        w_record.write("In %s, ab num: %d, unmap num: %d, ab precent: %.2f, unmap precent: %.2f\n" %(filename, count_ab, count_unmap, count_ab / count_total, count_unmap / count_total))
                    else:
                        w_record.write("In %s, ab num: %d, total num: %.2f\n" %(filename, count_ab, count_total))
                w_record.close()
                #出来是九个文件： 3_single_path_fin_normal, 3_single_path_fin_ab, 3_single_path_fin_unmap
                #               4_multi_path_fin_normal, 4_multi_path_fin_ab, 4_multi_path_fin_unmap
                #               5_has_ixp_ip_normal, 5_has_ixp_ip_ab, 5_has_ixp_ip_unmap

            if steps.__contains__('step 4'):
                print('step 4')
                TagStepInRecordFile(record_file_name, 'step 4')
                if cur_map_method.__contains__('_'):
                    #check trace with additional mapping                
                    w_record = open(record_file_name, 'a')
                    CombineFiles(['3_single_path_fin_ab', '3_single_path_fin_unmap'], '3_temp')
                    CombineFiles(['4_multi_path_fin_ab', '4_multi_path_fin_unmap'], '4_temp')
                    CombineFiles(['5_has_ixp_ip_ab', '5_has_ixp_ip_unmap'], '5_temp')
                    files = ['3_temp', '4_temp', '5_temp']
                    for filename in files:
                        (count_ab, count_unmap, count_total) = CheckTraces(filename, True, False, True) #step 4
                        w_record.write("In %s, ab num: %d, unmap num: %d, ab precent: %.2f, unmap precent: %.2f\n" %(filename, count_ab, count_unmap, count_ab / count_total, count_unmap / count_total))
                    w_record.close()
                    os.system('mv 3_temp_ab 3_single_path_fin_ab_s2')
                    os.system('mv 3_temp_unmap 3_single_path_fin_unmap_s2')
                    os.system('mv 4_temp_ab 4_multi_path_fin_ab_s2')
                    os.system('mv 4_temp_unmap 4_multi_path_fin_unmap_s2')
                    os.system('mv 5_temp_ab 5_has_ixp_ip_ab_s2')
                    os.system('mv 5_temp_unmap 5_has_ixp_ip_unmap_s2')
                    os.system('cat 3_single_path_fin_normal 3_temp_normal > 3_single_path_fin_normal_s2')
                    os.system('cat 4_multi_path_fin_normal 4_temp_normal > 4_multi_path_fin_normal_s2')
                    os.system('cat 5_has_ixp_ip_normal 5_temp_normal > 5_has_ixp_ip_normal_s2')
                    os.remove('3_temp')
                    os.remove('4_temp')
                    os.remove('5_temp')
                else:
                    shutil.copyfile('3_single_path_fin_ab','3_single_path_fin_ab_s2')
                    shutil.copyfile('3_single_path_fin_unmap','3_single_path_fin_unmap_s2')
                    shutil.copyfile('4_multi_path_fin_ab','4_multi_path_fin_ab_s2')
                    shutil.copyfile('4_multi_path_fin_unmap','4_multi_path_fin_unmap_s2')
                    shutil.copyfile('5_has_ixp_ip_ab','5_has_ixp_ip_ab_s2')
                    shutil.copyfile('5_has_ixp_ip_unmap','5_has_ixp_ip_unmap_s2')
                    shutil.copyfile('3_single_path_fin_normal','3_single_path_fin_normal_s2')
                    shutil.copyfile('4_multi_path_fin_normal','4_multi_path_fin_normal_s2')
                    shutil.copyfile('5_has_ixp_ip_normal','5_has_ixp_ip_normal_s2')
            
            #2021.5.17 将同一dstas匹配移到后面的模糊匹配中
            #if steps.__contains__('step 5'):
                #2021.3.2 在改为使用prefix进行匹配后，看一看，匹配同一dst_as，不同dst_prefix的情况有多少
                #print('step 5')
                #TagStepInRecordFile(record_file_name, 'step 5')
                #files = ['3_single_path_fin_ab_ab', '4_multi_path_fin_ab', '5_has_ixp_ip_ab_ab']
                #CombineFiles(files, 'ab_filter0')
                #(count_ab, count_total) = CheckAbPathByDstAs('ab_filter0', 'ab_filter1')
                #w_record = open(record_file_name, 'a')
                #w_record.write("In ab_filter1, ab num: %d, ab precent: %.2f\n" %(count_ab, count_ab / count_total))
                #w_record.close()

            if steps.__contains__('close_step'):
                print('close_step')
                TagStepInRecordFile(record_file_name, 'close_step: combine files')
                CombineFiles(['3_single_path_fin_ab_s2', '4_multi_path_fin_ab_s2', '5_has_ixp_ip_ab_s2'], 'final_ab')
                CombineFiles(['3_single_path_fin_unmap_s2', '4_multi_path_fin_unmap_s2', '5_has_ixp_ip_unmap_s2'], 'final_unmap')
                CombineFiles(['3_single_path_fin_normal_s2', '4_multi_path_fin_normal_s2', '5_has_ixp_ip_normal_s2'], 'final_normal')
                print(record_file_name)
                with open(record_file_name, 'r') as f:
                    res = f.read()
                #print(res)
                re_res = re.findall('Total valid trace num: (\d.*)', res)
                count_total = int(re_res[0])
                count_ab = len(open('final_ab', 'r').readlines()) / 3
                count_unmap = len(open('final_unmap', 'r').readlines()) / 3
                w_record = open(record_file_name, 'a')
                w_record.write("In total, ab num: %d, unmap num: %d, ab precent: %.4f, unmap precent: %.4f\n" %(count_ab, count_unmap, count_ab / count_total, count_unmap / count_total))
                w_record.close()
...
def tmp():
    ...
    ConnectToDb()
    SetCurMidarTableDate(year, month)
    bgp_filename = global_var.par_path + global_var.rib_dir + 'bgpdata/bgp_' + asn + '_' + date
    GetPfx2ASByBgp(bgp_filename) #2021.3.2使用VP本身的bgp表进行匹配，而不是用routeviews-rv2-20190301-1200.pfx2as_coalescedGetSibRel()
    GetBgpByPrefix(bgp_filename) #step 2 #get bgp_path to dst_prefix
    GetPathAsDict(bgp_filename) #step 3 #get bgp_path to dst_as
    CheckTraces('tmp_test1', False, False)

def StatAvg(filename, grep_key, val_key, threshold):
    #os.chdir(global_var.par_path + global_var.out_my_anatrace_dir + '/')
    #cmd = 'grep \'' + grep_key + '\' -r */ribs_midar_bdrmapit/' + filename + '*'
    os.chdir(global_var.par_path + 'tmp_out_my_anatrace/')
    cmd = 'grep \'' + grep_key + '\' -r */ribs_bdrmapit/' + filename + '*'
    output = os.popen(cmd)
    ...
    num = 0
    while data:
        vp = data[:data.index('_')]
        if vp == 'ord-us':
            data = output.readline()
            continue
        if vp not in vp_sum_dict.keys():
            vp_sum_dict[vp] = [0, 0] 
        find_res = re.findall(val_key + '.*?(\d+(\.\d+)?)', data)
        if find_res:
            val = float(find_res[0][0])
            if val > threshold:
                print('ab val: ' + data)
            else:
                vp_sum_dict[vp][0] += val
                vp_sum_dict[vp][1] += 1
                sum += val
                num += 1
        data = output.readline()
    for vp in vp_sum_dict.keys():
        print(vp)
        print('%.2f' %(vp_sum_dict[vp][0]/vp_sum_dict[vp][1]))
    print('total avg')
    print('%.2f' %(sum / num))

def StatAvg_2(filename, grep_key, data_key, threshold):
    # os.chdir(global_var.par_path + global_var.out_my_anatrace_dir + '/')
    # cmd = 'grep \'' + grep_key + '\' -r */ribs_midar_bdrmapit/' + filename + '*'
    os.chdir(global_var.par_path + 'tmp_out_my_anatrace/')
    cmd = 'grep \'' + grep_key + '\' -r */ribs_midar_bdrmapit/' + filename + '*'
    ...
    while data:
        ...
        if vp not in vp_sum_dict.keys():
            vp_sum_dict[vp] = [0, 0] 
        #find_res = re.findall('%s(\d+(\.\d+)?)'%data_key, data)
        find_res = re.findall('%s(\d+?)'%data_key, data)
        if find_res:
            #val = float(find_res[0][0])
            val = float(find_res[0])
            if val > threshold:
                print('ab val: ' + data)
            else:
                vp_sum_dict[vp][0] += val
    ...
    for vp in vp_sum_dict.keys():
        print(vp)
        print('\t%.1f%%' %(vp_sum_dict[vp][0] * 100 / vp_sum_dict[vp][1]))
    print('total avg')
    print('\t%.1f%%' %(sum * 100 / num))

def main_func():
    #FilterOneAbBetweenSameNormal('test1')
    PreGetSrcFilesInDirs()
    wf_join_org = open('tmp_join_org', 'w')
    for year in range(2020,2021):
    #for year in range(2019,2020):
        for month in range(1,13):
        #for month in range(1, 2):
            if (year == 2016 and month < 4) or (year == 2019 and month < 4): #2016年4月前peeringdb数据不准，2020年5月后的数据不全
                continue
            #if not (year == 2019 and month == 4):
                #continue
            #RunOneDate(global_var.vps, year, month, global_var.map_methods)
            StatMatchIp(global_var.vps, year, month, global_var.map_methods)
            #LooseCheck(global_var.vps, year, month, wf_join_org)

def StatMmLocation(filename):
    total_locs = []
    ...
    with open(filename, 'r') as rf:
        ...
        while curline_trace:
            curline_bgp = rf.readline()
            curline_ip = rf.readline()
            trace_list = curline_trace[curline_trace.index(']')+2:-1].split(' ')
            bgp_list = curline_bgp.strip('\n').strip('\t').split(' ')
            mismatch_locs = []
            last_match_bgp_index = 0
            for hop in trace_list:
                if hop == '*' or hop == '?' or hop == '<>':
                    continue
                find = False
                for uni_asn in hop.split('_'):
                    if uni_asn in bgp_list:
                        find = True
                        last_match_bgp_index = bgp_list.index(uni_asn) + 2
                        break
                if not find:
                    mismatch_locs.append(last_match_bgp_index)
            total_bgp_num = len(bgp_list)
            # for loc in mismatch_locs:
            #    total_locs.append(loc / total_bgp_num)
            if mismatch_locs:
                total_locs.append(mismatch_locs[0] / total_bgp_num)
            curline_trace = rf.readline()
    ave = np.mean(total_locs)
    med = np.median(total_locs)
    print("%.2f, %.2f" %(ave, med))

if __name__ == '__main__':
    vp = 'sjc2-us'
    for i in range(1, 13):
        StatMmLocation('/mountdisk1/ana_c_d_incongruity/out_my_anatrace/' + vp + '_2019' + str(i).zfill(2) + '15/ribs/final_ab')
    print('\n')
    for i in range(1, 13):
        StatMmLocation('/mountdisk1/ana_c_d_incongruity/out_my_anatrace/' + vp + '_2019' + str(i).zfill(2) + '15/bdrmapit/final_ab')
    #main_func()
    #tmp()
    #StatAvg_2('record3_', 'In check_use_irr', 'ab num: ', 500000)
