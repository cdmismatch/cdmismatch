...
def AddIxpFlagToTracePath(ori_trace_path, ip_path):
    global cur_map_method
    if not ori_trace_path or not ip_path:
        print(ori_trace_path)
        print(ip_path)
        return ori_trace_path
    #print(ip_path)
    trace_list = ori_trace_path.strip('\n').strip(' ').split(' ')
    ip_list = ip_path.strip('\n').strip(' ').split(' ')
    #print('ip_path: %s' %ip_path)
    for i in range(0, len(trace_list)):
        if ip_list[i] == '*':
            continue
        if IsIxpIp(ip_list[i]):
            trace_list[i] = '<>'
        else:
            for asn in trace_list[i].split('_'):
                if IsIxpAs(asn):
                    trace_list[i] = '<>'
                    break
...
def TracePathIsNormal_WithSib(ori_trace_path, ori_bgp_path, ip_path):
    global cur_map_method
    match_trace_list = []
    bgp_path_list = CompressAsPathToMin(ori_bgp_path).split(' ')
    ori_trace_path_list = ori_trace_path.split(' ')
...
def CheckTraces_WithIxp(filename):
    rf = open(filename, 'r')
    ...
    while curline:
        ...
        similar_bgp_path = ''
        ori_trace_path_2 = AddIxpFlagToTracePath(ori_trace_path, ip_path)
        ...
        for ori_bgp_path in bgp_path_list:
            (normal, dif_size, match_trace_path) = TracePathIsNormal(ori_trace_path_2, ori_bgp_path, ip_path, False)
            if normal:
...
def CheckTracesWithSib(filename):
    rf = open(filename, 'r')
    wf_ab = open(filename + '_ab', 'w')
    wf_unmap = open(filename + '_unmap', 'w')
    wf_normal = None
    ...
    while curline:
        ...
        match_trace_path = ''
        for ori_bgp_path in bgp_path_list:
            (normal, dif_size, match_trace_path) = TracePathIsNormal_WithSib(ori_trace_path_2, ori_bgp_path, ip_path)
            if normal:
                similar_bgp_path = ori_bgp_path
                break
            if min_dif_size > dif_size:
...
def StatMatchIp(vps, year, month, map_methods):
    ...
    for vp in vps:
        cur_dir = global_var.par_path +  'tmp_out_my_anatrace/' + vp + '_' + date + '/' #'nrt-jp.2019030115/'
        print(cur_dir)
        ...
        for tmp_map_method in map_methods:
            ...
            print(cur_map_method)
            #if cur_map_method.__contains__('ribs'):
            if not os.path.exists(cur_map_method):
                continue
            os.chdir(cur_map_method)
            record_file_name = 'record4_' + vp + '.' + date + '_' + cur_map_method
            #os.system("pwd")
...
def LooseCheck(vps, year, month, wf_join_org = None):
    steps = ['norm_trace_asn', 'find_similarest_bgp', 'check_use_newer_org_data', 'check_use_irr', 'oneab_betw_samenorm'] #2021.7.11，'same_dst_as'几乎没有过滤掉trace，而且即使匹配也不能说明trace正常，这里不再检查
    #steps = ['oneab_betw_samenorm']
    year_str = str(year)
    month_str = str(month).zfill(2)
    date = year_str + month_str + '15'
...
def RunOneDate(vps, year, month, map_methods):
    #steps = ['step 1', 'step 2', 'step 3', 'step 4', 'close_step', 'step ixp', 'step sib']
    ...
    if steps.__contains__('step ixp') or steps.__contains__('step sib'):
        GetSibRel(year, month)
        GetIxpPfxDict(year, month)
        GetIxpAsSet()   #这条语句需要放在GetSibRel()之后，因为需要sib_dict，也就是as2org_dict
    ConnectToDb()
    ...
    for vp in vps:
        ...
        global cur_map_method
        if steps.__contains__('step ixp') or steps.__contains__('step sib'):
            map_methods = ['ribs_midar_bdrmapit']
        match_stat = dict()
        for tmp_map_method in map_methods:
            cur_map_method = tmp_map_method
            print(cur_map_method)
            ...
            if steps.__contains__('step 1'):
                ...
                if cur_map_method.__contains__('_') and os.path.isfile('../ribs/0_no_bgp'): #联合方法，且前面已经有ribs记录，这里直接copy，不用再计算
                    ...
                    wf = open(record_file_name, 'w')
                    rf = open('../ribs/record4_' + vp + '.' + date + '_ribs', 'r')
                    curline = rf.readline()
            ...
            if steps.__contains__('step 3'):
                ...
                files = ['3_single_path_fin', '4_multi_path_fin', '5_has_ixp_ip']
                for filename in files:
                    (count_ab, count_unmap, count_total) = CheckTraces(filename, True, False, True) #step 2
                    if count_total > 0:
            ...
            if steps.__contains__('step ixp'):
                print('step ixp')
                TagStepInRecordFile(record_file_name, 'step ixp')
                w_record = open(record_file_name, 'a')
                (count_ab, count_unmap, count_total) = CheckTraces_WithIxp('final_ab') #step 2
                if count_total > 0:
                    w_record.write("In ixp, ab num: %d, unmap num: %d, ab precent: %.2f, unmap precent: %.2f\n" %(count_ab, count_unmap, count_ab / count_total, count_unmap / count_total))
                else:
                    w_record.write("In ixp, ab num: %d, total num: %.2f\n" %(count_ab, count_total))
                w_record.close()
            
            if steps.__contains__('step sib'):
                print('step sib')
                TagStepInRecordFile(record_file_name, 'step sib')
                w_record = open(record_file_name, 'a')
                (count_ab, count_unmap, count_total) = CheckTracesWithSib('final_ab_ab') #step 2
                if count_total > 0:
                    w_record.write("In sib, ab num: %d, unmap num: %d, ab precent: %.2f, unmap precent: %.2f\n" %(count_ab, count_unmap, count_ab / count_total, count_unmap / count_total))
                else:
                    w_record.write("In sib, ab num: %d, total num: %.2f\n" %(count_ab, count_total))
                w_record.close()
